...
> 系统在功能正确之后，下一个挑战就是"扛不扛得住"。本章回答：高并发问题的本质是什么？如何用流量模型量化系统容量？水平扩展和无状态设计的工程实践是什么？经典高并发架构的每一层各自解决什么问题？
...
└────┬────┘
     ▼
┌─────────┐   CPU: 序列化/反序列化、业务计算
│  应用服务 │   内存: 对象分配、Session
│  (JVM)  │   网络IO: 调用数据库、Redis、RPC
└────┬────┘
     ├──────────┬──────────┐
     ▼          ▼          ▼
┌────────┐ ┌────────┐ ┌────────┐
...
上面的代码用了 Guava RateLimiter（令牌桶），但限流算法不止一种。选择哪种取决于场景：
...
> **高并发是架构能力的试金石。** 本章从资源竞争的本质出发，用流量模型量化容量，以无状态设计支撑水平扩展，通过全链路逐层消峰实现系统韧性。这些模式与第1章的分层架构思想一脉相承——每一层都是一个"保护层"，将不可控的外部流量转化为可控的内部处理。结合第2章的 DDD 领域建模，一个既健壮又可维护的系统骨架便初具雏形。
