...
Order                              orders 表
├── buyer ──→ User                 ┌────┬─────────┬──────────┐
│   └── address ──→ Address        │ id │ buyer_id│ status   │
└── items ──→ List<OrderItem>      ├────┼─────────┼──────────┤
    ├── OrderItem                  │ 1  │   100   │ PAID     │
    └── OrderItem                  └────┴─────────┴──────────┘
                                              │
                                   users 表    │ 外键关联
                                   ┌────┬──────┼───┬─────┐
...
├─────────┼───────────────────────────────────────────────────────┤
│ 第5章   │ 数据库原理 —— 索引、执行计划、连接池、分库分表             │
├─────────┼───────────────────────────────────────────────────────┤
│ 第6章   │ Spring 事务管理 —— 声明式事务、传播行为、隔离级别          │
├─────────┼───────────────────────────────────────────────────────┤
│ 第7章   │ 性能优化 —— 批量操作、缓存策略、慢查询排查、读写分离       │
└─────────┴───────────────────────────────────────────────────────┘
```
...
- 如果你关注 **系统性能**，第 5、7 章是重点——索引优化和查询调优是日常开发中最容易产生价值的技能。
- 如果你对 **分布式事务** 感兴趣，第 6 章的事务基础是后续理解分布式事务的前提。
...
> **纵横联系**
>
> - **向上关联**：本章讨论的持久化问题，在《Java 并发编程》卷中有对应的并发数据访问挑战——多线程环境下如何安全地读写数据库？这将在第 6 章（事务管理）中展开。
> - **向下关联**：JDBC 的底层是 TCP Socket 通信和数据库协议（如 MySQL 的 MySQL Protocol），理解网络 I/O 模型有助于理解连接池的设计原理。这将在第 5 章（数据库原理）中触及。
> - **横向关联**：对象-关系阻抗失配的思想，与《设计模式》中的适配器模式（Adapter）、建造者模式（Builder）高度相关——ORM 框架本质上就是对象世界和关系世界之间的"巨型适配器"。
> - **向前展望**：了解了持久化的本质问题后，后续章节将从 JDBC 这个"底层原语"开始，逐步向上构建，直到你能在 Spring 项目中游刃有余地处理各种数据访问场景。
