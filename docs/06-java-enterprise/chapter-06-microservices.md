...
### 6.3.2 Spring Cloud Gateway 核心概念
...
> **卷间纵横联系**
>
> - **纵向**：本章是第六卷的架构篇终章，将前几章的 Spring Boot（应用构建）、数据访问（存储层）串联为完整的微服务体系。后续章节将进入分布式事务、消息队列等更深入的分布式问题。
> - **横向**：对比第一卷《Java 基础》中的动态代理机制，OpenFeign 和 Dubbo 的服务代理都基于 JDK 动态代理或字节码生成。对比第二卷《JVM》中的类加载机制，Spring Boot 的自动配置依赖 classpath 扫描能力。对比第三卷《数据库与存储》中的事务 ACID，微服务架构下的分布式事务（如 Seata）是事务管理的更高维度挑战。
