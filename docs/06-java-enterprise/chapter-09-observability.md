...
> **纵横联系**
>
> - **与第六卷第 7 章《分布式系统治理》的关系**：链路追踪是治理的核心手段之一，第 7 章介绍了 SkyWalking 的基础使用，本章从可观测性全局视角将追踪与日志、指标串联，形成完整的观测体系
> - **与第六卷第 8 章《企业系统安全与部署》的关系**：K8s 的 liveness/readiness 探针产生的指标可以接入 Prometheus 监控；审计日志是日志体系的一部分；Docker 容器的日志收集需要适配容器环境
> - **与第五卷《数据访问》的关系**：慢 SQL 是接口变慢的最常见原因之一，链路追踪可以精确到每条 SQL 的执行耗时，指标监控可以观察数据库连接池状态
> - **与第六卷第6章《微服务架构》的关系**：微服务架构放大了可观测性的难度——一个请求跨越多个服务，必须依靠分布式追踪才能定位问题。本章的排查方法论是微服务运维的必备技能
