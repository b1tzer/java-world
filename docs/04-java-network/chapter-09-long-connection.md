...
> HTTP 的请求-响应模型天然适合"一问一答"的场景，但当服务端需要主动向客户端推送消息、或者双方需要高频低延迟地交换数据时，短连接模型就显得力不从心。本章要回答三个核心问题：为什么需要长连接？主流的长连接技术（WebSocket、SSE）各自适用于什么场景？如何在生产环境中保证长连接的稳定性，并设计一个可靠的 IM 系统？
