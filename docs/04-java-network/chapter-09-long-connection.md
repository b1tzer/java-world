...
> 你的 IM 服务上线三个月，用户反馈「消息偶尔收不到」。日志里一切正常——没有异常、没有超时、没有断连。你查了两天，最后在 `ss -ant` 里看到几千个 CLOSE_WAIT 连接——客户端断网了但服务端不知道，WebSocket 连接成了「僵尸连接」，心跳没设，消息发到 fd 上直接丢了。本章从你写的 `@OnMessage` 顺着 WebSocket 协议往回拆：长连接怎么建立、怎么维持、怎么发现对方断掉。
