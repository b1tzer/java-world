...
> **纵横联系**
>
> - **向前依赖**：本章的可见性抽象承接第 2 章"线程是并发执行单元"；对"共享数据"的处理是对第 3 章 `ThreadLocal`（不共享的路线）的正面回答。
> - **向后使用**：第 5 章会用 `volatile` 把本章的可见性与有序性落到单个变量的读写边界上；第 6 章会用 `synchronized` 把 happens-before 落到临界区上；第 7 章的 CAS、第 8 章的 AQS、第 9 章的并发集合都建立在 happens-before 之上；第 12 章的虚拟线程改变了调度方式，但本章的 JMM 规则完全不变。
> - **跨卷关系**：第一卷《Java 语言基础》中 `final` 的语法层含义，在本章 §4.5 得到并发语义补齐；第二卷《JVM Runtime》讨论 JIT 编译器如何具体实现内存屏障、逃逸分析对锁优化的影响；第七卷讨论分布式一致性时的 CAP 与线性化，则是把本章的规则从"单 JVM 内"推广到"跨节点"的对应版本。
