...
并发的好处是显而易见的。但天下没有免费的午餐——并发引入了单线程程序中根本不存在的问题。
...
| 章节 | 主题 | 核心问题 | 依赖前置 |
| :-- | :-- | :-- | :-- |
| 第2章 | 线程模型 | 线程是如何被创建、调度和销毁的？ | 无 |
| 第3章 | Java 内存模型 | 线程之间如何看到彼此的写入？ | 第2章 |
| 第4章 | volatile | 轻量级同步机制能保证什么、不能保证什么？ | 第3章 |
| 第5章 | synchronized | Java 内置锁的本质是什么？锁如何升级？ | 第3章、第4章 |
| 第6章 | Lock 与 AQS | synchronized 不够用时怎么办？AQS 如何统一并发工具？ | 第5章 |
| 第7章 | 原子类与 CAS | 无锁并发如何实现？CAS 的局限是什么？ | 第3章 |
| 第8章 | 并发集合 | 如何在高并发下安全地使用集合？ | 第6章、第7章 |
| 第9章 | 线程池 | 线程太多怎么办？如何复用和管理线程？ | 第6章、第8章 |
| 第10章 | 异步编程模型 | 从线程到异步，有哪些并发范式？ | 第9章 |
| 第11章 | 诊断与优化 | 并发问题怎么查？性能怎么优化？ | 全部前置 |

**学习建议：** 第2-3章是地基，必须扎实。第4-6章是核心同步机制，理解它们才能读懂 `java.util.concurrent` 的源码。第7-9章是应用层，日常开发中用得最多。第10章是范式扩展，第11章是工程实践。
...
建议按顺序阅读。如果你已经有一定基础，可以跳过前两章，从第3章（JMM）开始——它是理解后续所有内容的钥匙。

---

> **与本卷其他章节的关系：** 本章回答的是"为什么"的问题——为什么需要并发。接下来的第2章将回答"是什么"——Java 中的线程到底是什么东西；第3章回答"怎么保证正确性"——当多个线程同时访问数据时，Java 内存模型提供了哪些保证。如果你对本章提到的任何概念（如 CAS、AQS、ForkJoinPool）感到陌生，不必担心，后续章节会逐一展开。
