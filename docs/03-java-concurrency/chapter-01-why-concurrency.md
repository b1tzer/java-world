...
| 第4章 | 同步机制 | 如何保证同一时刻只有一个线程执行临界区？ | 第3章 |
| 第5章 | 显式锁与同步器 | synchronized 不够用时怎么办？ | 第4章 |
| 第6章 | 并发容器 | 如何在不加锁的情况下实现线程安全的集合？ | 第5章 |
| 第7章 | 线程池 | 线程太多怎么办？如何复用线程？ | 第5章、第6章 |
| 第8章 | 异步编程 | 如何用非阻塞的方式组织并发任务？ | 第7章 |
| 第9章 | 虚拟线程 | JDK 21 如何用轻量级线程颠覆传统模型？ | 第2章、第7章 |

**学习建议：** 第2-3章是地基，必须扎实。第4-5章是核心机制，理解它们才能读懂 `java.util.concurrent` 的源码。第6-8章是应用层，日常开发中用得最多。第9章是未来趋势，值得提前了解。
