...
问题显而易见：如果 `handleRequest` 耗时 100ms，那么每秒最多处理 10 个请求。第二个客户端必须等第一个请求处理完才能被接受。用户体验极差。
...
并发的好处是显而易见的，但代价同样明确——它引入了单线程程序中根本不存在的问题。

### 1.3.1 数据竞争：count++ 的陷阱
...
| 章节 | 主题 | 核心问题 | 依赖前置 |
| :-- | :-- | :-- | :-- |
| 第2章 | 线程：Java 的执行单元 | 线程是如何被创建、调度和销毁的？ | 无 |
| 第3章 | 线程封闭：`ThreadLocal` | 每条线程持有独立副本，如何避免竞争？ | 第2章 |
| 第4章 | Java 内存模型（JMM） | 线程之间如何看到彼此的写入？ | 第2章 |
| 第5章 | `volatile` | 轻量级同步机制能保证什么、不能保证什么？ | 第4章 |
| 第6章 | `synchronized` | Java 内置锁的本质是什么？锁如何升级？ | 第4章、第5章 |
| 第7章 | CAS 与原子类 | 无锁并发如何实现？CAS 的局限是什么？ | 第4章 |
| 第8章 | `LockSupport` 与 AQS | `synchronized` 不够用时怎么办？AQS 如何统一并发工具？ | 第6章、第7章 |
| 第9章 | 并发集合 | 如何在高并发下安全地使用集合？ | 第7章、第8章 |
| 第10章 | 线程池 | 线程太多怎么办？如何复用和管理线程？ | 第8章、第9章 |
| 第11章 | 异步编程 | 从 `Future` 到 `CompletableFuture`，有哪些异步范式？ | 第10章 |
| 第12章 | 虚拟线程与结构化并发 | JDK 21 之后，并发模型如何被重塑？ | 第2章、第10章、第11章 |
| 第13章 | 诊断与优化 | 并发问题怎么查？性能怎么优化？ | 全部前置 |

**学习建议：** 第2-4章是地基，必须扎实。第5-8章是核心同步机制，理解它们才能读懂 `java.util.concurrent` 的源码。第9-10章是应用层，日常开发中用得最多。第11-12章是范式扩展，第13章是工程实践。
...
第4章 ────→  ┌─────────┐             │          │             │          │
            ...
            └─────────┘             │          │             │          │
            ┌─────────┐  第5章 ───→  │volatile  │             │          │
            │         │  第6章 ───→  │sync/Mon  │  第9章 ───→  │ 并发容器  │
            │         │  第7章 ───→  │CAS/Atomic│  第10章 ──→  │ 线程池    │
            │         │  第8章 ───→  │Lock/AQS  │  第11章 ──→  │ 异步编程  │
            │         │             │          │  第12章 ──→  │ 虚拟线程  │
            └─────────┘             └──────────┘  第13章 ──→  │ 诊断优化  │
                                                             └──────────┘
```

建议按顺序阅读。如果你已经有一定基础，可以跳过前两章，从第 4 章（JMM）开始——它是理解后续所有内容的钥匙。

---

> **与本卷其他章节的关系：** 本章回答的是"为什么"的问题——为什么需要并发。第 2 章将回答"是什么"——Java 中的线程到底是什么东西；第 3 章引入线程封闭思路，先给出一条回避竞争的捷径；第 4 章回答"怎么保证正确性"——当多个线程同时访问数据时，Java 内存模型提供了哪些保证。如果你对本章提到的任何概念（如 CAS、AQS、ForkJoinPool、虚拟线程）感到陌生，不必担心，后续章节会逐一展开。
