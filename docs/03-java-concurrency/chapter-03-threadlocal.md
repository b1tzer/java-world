...
> **纵横联系**
>
> - **向前（第 2 章）**：本章的每一条推理都建立在"线程 = 独立执行流、独立栈"的基础上；`Thread.threadLocals` 字段挂在 `Thread` 对象上，正是线程模型的自然延伸。
> - **向后（第 4 章 JMM）**：线程封闭天然满足可见性与有序性——因为根本没有跨线程写入，happens-before 无从违反。第 4 章会解释另一条路径：跨线程共享时如何靠规范建立同步。
> - **向后（第 10 章 线程池）**：Worker 长期复用是本章内存泄漏叙事的关键放大器；线程池章节会从另一个角度分析 Worker 生命周期。
> - **向后（第 12 章 虚拟线程）**：百万级虚拟线程 × 每线程若干 TL 值意味着内存量级的重估；`ScopedValue`（JDK 21 预览）是 JDK 团队给出的替代方案。
> - **跨卷（第六卷 Spring）**：事务传播、`SecurityContextHolder`、`RequestContextHolder` 全部以 `ThreadLocal` 为底座；异步方法丢上下文的根因就在本章。
> - **跨卷（第七卷 分布式）**：跨 JVM 的上下文传播（trace 头、租户 ID、灰度标签）从进程内的 TTL 起步，止于协议层的 Baggage 头部。
