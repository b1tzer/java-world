...
> 本章建立 JVM 运行时数据区的完整认知：堆、虚拟机栈、本地方法栈、方法区、程序计数器各自存储什么、内部如何工作、什么条件下会出问题。
...
## 2.4 本地方法栈：连接 Java 世界与本地世界的执行环境
