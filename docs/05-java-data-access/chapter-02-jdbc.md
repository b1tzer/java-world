...
│           业务代码（你的 Java 代码）        │
├─────────────────────────────────────────┤
│     ORM 框架（MyBatis / Hibernate）       │  ← 第3、4章讲的
├─────────────────────────────────────────┤
│     JDBC（java.sql.*）                    │  ← 本章讲的
├─────────────────────────────────────────┤
│     JDBC 驱动（mysql-connector-java 等）   │
├─────────────────────────────────────────┤
│     数据库通信协议（TCP/IP）                │  ← 第四卷讲的
├─────────────────────────────────────────┤
│     数据库服务器（MySQL / Oracle / PG）     │
└─────────────────────────────────────────┘
```
...
### 2.2.3 PreparedStatement——SQL 的执行者
...
### 2.4.2 PreparedStatement 的参数化查询
...
除了安全性，`PreparedStatement` 还有性能优势。当你发送一条带 `?` 的 SQL 给数据库时，数据库会：

1. **解析 SQL**：检查语法、验证表和列的存在
2. **优化执行计划**：选择最优的索引和查询策略
3. **缓存执行计划**：相同的 SQL 模板可以复用执行计划

当你后续用不同的参数值再次执行同一条 SQL 时，数据库跳过步骤 1 和 2，直接使用缓存的执行计划。对于高频执行的查询（如根据 ID 查用户），这个优化非常显著。
...
> **纵横联系**
>
> - **向上**：本章是 JDBC 的"底层视角"。下一章（第 3 章 MyBatis）将展示如何在 JDBC 之上构建更优雅的 SQL 映射框架，把开发者从样板代码中解放出来。
> - **向下**：`Connection` 底层是一个 TCP Socket，连接池的设计直接对应第四卷《Java 网络与分布式通信》中 TCP 连接管理的知识。理解 TCP 三次握手和 Keep-Alive，才能真正理解连接池参数的含义。
> - **向右**：`PreparedStatement` 的预编译机制，是数据库查询优化器的工作基础。第 5 章（事务管理）和第 6 章（SQL 优化）将在此基础上展开更深入的讨论。
