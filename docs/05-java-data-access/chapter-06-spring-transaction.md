...
### 6.2.3 REQUIRED vs REQUIRES_NEW：一个经典面试场景
...
### 6.4.3 PlatformTransactionManager：更底层的控制
...
### 6.5.1 @Transactional 是 AOP 的"教科书案例"
...
> **纵横联系：**
> - **向前（第四卷《Web 开发基础》）**：Web 层的 `@Controller` 调用 `@Service`，事务就在 Service 层静静等待，确保数据一致性。
