...
> 一个 Java 应用的性能瓶颈，十有八九出在数据访问层。数据库连接的创建有多昂贵？一条 SQL 为什么会拖垮整个系统？连接池的参数该怎么调？本章从连接池原理出发，覆盖批处理、链路分析和常见问题排查，帮你建立一套系统化的数据访问性能优化方法论。
...
> **纵横联系**
>
> - **本卷第 5-6 章**：JDBC 和 ORM 框架是本章优化的基础——你得先知道数据怎么访问，才能知道怎么优化它
> - **第 3 卷《Spring Boot》**：连接池配置、AOP 监控、Actuator 指标采集，都依赖 Spring Boot 的自动配置体系
> - **第 4 卷《微服务》**：在微服务架构下，数据访问链路更长（网关 → 服务 → 数据库），链路追踪（SkyWalking / Zipkin）成为必备
> - **第 6 卷《系统设计与架构》**：当单库性能到顶时，读写分离、分库分表、缓存策略是下一阶段的解法
