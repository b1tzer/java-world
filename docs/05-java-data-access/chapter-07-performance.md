...
> - **第六卷《企业架构》**：连接池配置、AOP 监控、Actuator 指标采集，都依赖 Spring Boot 的自动配置体系
> - **第六卷《企业架构》**：在微服务架构下，数据访问链路更长（网关 → 服务 → 数据库），链路追踪（SkyWalking / Zipkin）成为必备
> - **第七卷《性能与架构》**：当单库性能到顶时，读写分离、分库分表、缓存策略是下一阶段的解法
