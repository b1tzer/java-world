...
> **本章小结**：高可用不是买来的，是设计出来的。量化可用性目标 → 消除单点 → 建立冗余 → 隔离故障 → 优雅降级，这是一个完整的高可用建设路径。记住：**没演练过的高可用 = 没有高可用**。

---

> 单机高可用靠冗余和故障转移。但当系统从一台机器变成几十台，"写入成功就能读到"这个基本假设突然不成立了。下一章从 CAP 理论出发，讲分布式环境下的一致性、事务和锁。
>
> **纵横联系**
>
> - **与第5章（分布式系统核心问题）**：冗余设计引出数据一致性问题——多副本之间如何保持同步？分布式事务如何保证跨服务操作的原子性？
> - **与第6章（数据架构）**：缓存层的高可用（Redis Cluster/Sentinel）是整体可用性链路中的关键一环，缓存击穿/雪崩本身就是可用性事件。
> - **与第三卷（微服务架构）**：服务拆分后的故障隔离、熔断降级是微服务治理的核心能力，Spring Cloud / Resilience4j 是主要技术选型。
> - **与第一卷（并发编程）**：线程池隔离本质上是对并发资源的精细管控，理解线程池参数对隔离策略的设计至关重要。
