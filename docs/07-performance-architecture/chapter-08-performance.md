...
> - **第一卷《Java 语言基础》**：JMH 基准测试依赖对 Java 语法和注解处理的理解
> - **第二卷《JVM 运行时》**：GC 调优、内存诊断直接建立在 JVM 内存模型之上
> - **第三卷《并发编程》**：线程池调优、锁竞争分析是并发知识的直接应用
> - **第四卷《网络与通信》**：I/O 模型选择直接影响吞吐量和延迟
> - **第五卷《数据访问》**：慢 SQL 优化、连接池配置是数据库性能的核心
> - **第六卷《企业架构》**：分布式链路追踪、服务网格的性能治理
> - **第九章《架构案例分析》**：秒杀系统的压测和调优是本章方法论的综合实战
