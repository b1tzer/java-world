...
> 本章建立 Java 世界观，同时回答"Java 到底如何表示一个对象"。不讲 Hello World，而是从设计目标出发，理解 Java 为什么存在，然后深入类型系统——这是整本书的地基。
...
Java 的类型世界分为两大阵营：基本类型（Primitive）和引用类型（Reference）。理解这个划分，是理解 Java 内存模型、对象模型、泛型的前提。
