...
### 2.3.4 CompletableFuture（JDK 8.0）
...
> - **第二卷《JVM 深度》**：HotSpot 的线程实现、栈帧结构、线程本地分配缓冲区（TLAB）等内容，与本章的 1:1 模型直接相关
